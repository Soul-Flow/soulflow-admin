import { create } from "zustand";
import { SortOrder } from "@/enums/sort-order.enum";
import type { CartResponse } from "@/interfaces/responses/cart-response.interface";
import type { PageResponse } from "@/interfaces/responses/page-response.interface";
import type { CartRequest } from "@/interfaces/resquests/cart-resquest.interface";
import { cartService } from "@/services/cartService";

interface CartState {
	pages: Map<string, PageResponse<CartResponse> | undefined>;

	loading: boolean;

	save: (request: CartRequest) => Promise<CartResponse>;

	deleteByPk: (pk: number) => Promise<void>;

	findByPk: (
		params: {
			keyword: string | null;
			fromDate: string | null;
			toDate: string | null;
			expired: boolean;
			deleted: boolean;
			sortOrder: SortOrder;
			pageNumber: number;
			pageSize: number;
		},
		pk: string,
	) => Promise<CartResponse | undefined>;

	filter: (params: {
		keyword: string | null;
		fromDate: string | null;
		toDate: string | null;
		expired: boolean;
		deleted: boolean;
		sortOrder: SortOrder;
		pageNumber: number;
		pageSize: number;
	}) => Promise<PageResponse<CartResponse> | undefined>;

	clearCache: () => void;
}

const useCartStore = create<CartState>((set, get) => ({
	pages: new Map<string, PageResponse<CartResponse>>(),

	loading: false,

	save: async (request: CartRequest): Promise<CartResponse> => {
		try {
			set({ loading: true });
			return await cartService.save(request);
		} catch (error) {
			console.log(error);
			throw error;
		} finally {
			set({ loading: false });
		}
	},

	deleteByPk: async (pk: number): Promise<void> => {
		try {
			set({ loading: true });
			await cartService.deleteByPk(pk);
		} catch (error) {
		} finally {
			set({ loading: false });
		}
	},

	findByPk: async (
		{
			keyword = null,
			fromDate = null,
			toDate = null,
			expired = false,
			deleted = false,
			sortOrder = SortOrder.DESC,
			pageNumber = 0,
			pageSize = 5,
		}: {
			keyword: string | null;
			fromDate: string | null;
			toDate: string | null;
			expired: boolean;
			deleted: boolean;
			sortOrder: SortOrder;
			pageNumber: number;
			pageSize: number;
		},
		pk: string,
	): Promise<CartResponse | undefined> => {
		const key = [
			keyword,
			fromDate,
			toDate,
			expired,
			deleted,
			sortOrder,
			pageNumber,
			pageSize,
		].join("_");

		const page = get().pages.get(key);

		if (page) {
			return page.content.find((e) => e.pk === pk);
		}

		return await cartService.findByPk(Number(pk));
	},

	filter: async ({
		keyword = null,
		fromDate = null,
		toDate = null,
		expired = false,
		deleted = false,
		sortOrder = SortOrder.DESC,
		pageNumber = 0,
		pageSize = 5,
	}: {
		keyword: string | null;
		fromDate: string | null;
		toDate: string | null;
		expired: boolean;
		deleted: boolean;
		sortOrder: SortOrder;
		pageNumber: number;
		pageSize: number;
	}): Promise<PageResponse<CartResponse> | undefined> => {
		const key = [
			keyword,
			fromDate,
			toDate,
			expired,
			deleted,
			sortOrder,
			pageNumber,
			pageSize,
		].join("_");

		const page = get().pages.get(key);

		if (page) {
			set((state) => {
				const newMap = new Map(state.pages);
				newMap.delete(key);
				newMap.set(key, page);
				return { pages: newMap };
			});
			return page;
		}

		const newPage = await cartService.filter({
			keyword,
			fromDate,
			toDate,
			expired,
			deleted,
			sortOrder,
			pageNumber,
			pageSize,
		});

		set((state) => {
			const newMap = new Map(state.pages);
			if (newMap.size >= 10) {
				const firstKey = newMap.keys().next().value;
				if (firstKey) newMap.delete(firstKey);
			}
			newMap.set(key, newPage);
			return { pages: newMap };
		});
		return newPage;
	},

	clearCache: () => {
		set({ pages: new Map() });
	}
}));

export default useCartStore;
